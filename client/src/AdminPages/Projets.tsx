...
function Projets() {
  const { projData, setProjData } = useContextProvider();
  ...
  const handleEditClick = async (
    ...
    console.info(id);
    const updateProj = projData.find((p) => p.id === id);
    if (!updateProj) {
  ...
  const handleEditKeyPressClick = async (id: number) => {
    if (edit === true) {
      try {
        ...
        console.info("données certdata au clic envoyé en PUT =>", projData);

        if (response.ok) {
          const projsData = await response.json();
          console.info(
            "données modifiées reçues et enregistrées du back certData => ",
            projsData,
          );
          setProjData(projsData);
          toggleSwitch();
        } else {
          console.error(
  ...
  const fetchProjData = async () => {
    try {
      ...
      if (response.ok) {
        const projsData = await response.json();
        setProjData(projsData);
        console.info("données fetchées certData =>", projsData);
      }
    } catch (err) {
